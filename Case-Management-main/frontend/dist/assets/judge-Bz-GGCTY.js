import"./animations-BWX3bC_M.js";import{c as b,g as y,a as C}from"./web3Service-BnnceUp4.js";const g=localStorage.getItem("token"),x=localStorage.getItem("user");let n=null,l=[],r=[],c=null,f=!1;(!g||!x)&&(window.location.href="login.html");let o;var v;try{o=JSON.parse(x),o.role!=="judge"&&(alert("Unauthorized: You must be a judge."),window.location.href="index.html"),document.getElementById("user-name").innerText=`Hon. ${o.name||o.email.split("@")[0]}`,document.getElementById("user-role").innerText="ID: "+(o.badgeId||((v=o._id)==null?void 0:v.slice(-6).toUpperCase())||"N/A")}catch(t){console.error(t)}document.getElementById("logout-action").addEventListener("click",()=>{localStorage.clear(),window.location.href="index.html"});document.getElementById("connect-wallet-btn").addEventListener("click",async()=>{try{c=await b();const t=document.getElementById("connect-wallet-btn");t.innerText="Wallet Connected",t.style.borderColor="#60a5fa",t.style.color="#60a5fa";const e=document.getElementById("wallet-address");e.style.display="block",e.innerText=`${c.substring(0,6)}...${c.substring(38)}`,n&&h()}catch(t){console.error(t),alert(t.message||"Failed to connect wallet.")}});async function E(){try{const t=await fetch("http://localhost:5000/api/cases/allCases",{headers:{Authorization:`Bearer ${g}`}}),e=await t.json();if(!t.ok)throw new Error(e.message||"Failed to fetch");l=Array.isArray(e)?e:[],r=[...l],m()}catch(t){console.error(t),document.getElementById("case-list-container").innerHTML='<p style="color:red; text-align:center;">Error loading cases.</p>'}}function m(){const t=document.getElementById("case-list-container");if(document.getElementById("total-count").innerText=`${r.length} Items`,r.length===0){t.innerHTML='<p style="color: #666; text-align: center; margin-top: 20px;">No cases found.</p>';return}t.innerHTML=r.map(e=>{var a;return`
